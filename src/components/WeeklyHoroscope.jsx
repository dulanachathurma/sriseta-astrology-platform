...
function buildSlides(items) {
  ...
  // API දත්ත තිබේ නම් එය ගන්න, නැතිනම් lagnaData හි ඇති ස්ථිර විස්තරය ගන්න
  return lagnaData.map((lagna) => ({ 
    ...lagna, 
    text: bySign.get(lagna.name) || lagna.description 
  }));
}
...
export default function WeeklyHoroscope() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStatus('ready'); // Error එකකදීත් ස්ථිර විස්තර පෙන්වීමට 'ready' ලෙස තබන්න
    }
  }, []);
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="font-sinhala text-white/90 leading-relaxed text-lg">{slide.text}</p>
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
